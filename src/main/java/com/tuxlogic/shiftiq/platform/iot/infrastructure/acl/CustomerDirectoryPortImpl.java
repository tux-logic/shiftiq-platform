...
public class CustomerDirectoryPortImpl implements CustomerDirectoryPort {
    ...
    public List<UUID> findActiveCustomerIdsByBranchId(BranchId branchId) {
        var result = customerRegistrationQueryService.handle(branchId, CustomerRegistrationStatus.ACTIVE, Pageable.unpaged());
        return result.success().orElseGet(org.springframework.data.domain.Page::empty).getContent().stream()
